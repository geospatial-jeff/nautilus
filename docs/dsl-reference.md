...
| `.drop(*cols)` | Drop these columns, keeping the rest in their original order. |
...
raises when the stream runs, not when you build it.
...
`.union(other)` concatenates two streams into one — every row from both, duplicates kept, like SQL `UNION
ALL`. There is no key and no shuffle: each side's batches flow straight through, so it holds no state and
runs at any `parallelism`. The two inputs must share a schema; since nautilus learns schemas from the
data, a mismatch raises when the stream runs, not when you build it. A stream cannot be unioned with
itself — build the two sides as separate streams.

```python
all_events = source(clicks).union(source(views))
```
