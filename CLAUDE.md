...
**These documentation standards are a HARD REQUIREMENT of every change — not a suggestion, not a
nice-to-have, and not a cleanup pass you do later. Read them before you write code, and meet them in the
first draft.** If you are editing doc prose in a follow-up pass to bring it up to standard, you have
already failed this requirement. There is no separate "docs later" step. Write it right the first time.

## Documentation is part of every change

You MUST apply the standards below from the **first draft** — when you write a plan (`DESIGN.md`,
`IMPLEMENTATION_PLAN.md`, or a working plan file) and when you write the initial module/function
docstring, inline comment, or reference-doc entry for new code. DO NOT draft prose loosely and tidy it
up afterward. When a plan stages a deliverable, the docstrings, comments, and reference/glossary updates
that deliverable needs ARE part of that deliverable — name them in the plan and write them as you build.

When you change code, you MUST update every doc the change touches in the same pass — including ones you
did not write. A docstring or DESIGN.md sentence that the change makes false is a defect; leaving it is
not allowed.
...
the *what-not-to-do*. A sentence that restates the code or the file tree is deletable — delete it.
...
This is the rule you must not break.
...
Before you call a change done, re-read the docs you touched and confirm: no fact appears at two
altitudes; nothing restates the code; nothing the change made false survives. If a fact is repeated,
delete it from every altitude but the right one.

### Rules — each is mandatory
...
- **Concrete over abstract.** "counter, gauge, histogram" beats "instruments." NEVER swap a familiar
  word for a jargon umbrella to save space — that raises what the reader must already know.
- **Justify every fact for its reader.** DO NOT keep a detail just because it was in the sentence you
...
- **Match the code's words.** Use the exact names the code uses; never substitute synonyms.
- **Never edit generated docs.** `docs/telemetry-reference.md` is generated from the catalog — change
